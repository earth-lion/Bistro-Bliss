...
const botTranslations = {
  en: {
    ...
    apiHelp:
      "Enter your Gemini API key to unlock advanced AI conversation, dietary recommendations, and smart menu suggestions.",
    ...
    welcome:
      "👋 Hello! I'm your **Bistro AI Companion**. I can help you explore our menu, find dishes by taste or category, book a table, or answer any question about Bistro Bliss!",
    noMatches:
      "I couldn't find a direct match. Try asking about a dish name, category (pizza, pasta, burger), or tap a quick chip below!",
    ...
    welcomeFeatures: [
      {
        icon: "🍽️",
        title: "Explore Menu",
        desc: "Browse all dishes by category",
      },
      {
        icon: "🤖",
        title: "AI Powered",
      ...
      },
      {
        icon: "🛒",
        title: "Quick Order",
        desc: "Add dishes directly from chat",
      },
    ],
    quickChips: {
      menu: "🍕 Menu",
  ...
  ar: {
    ...
    apiHelp:
      "أدخل مفتاح Gemini API لتفعيل المحادثات الذكية المتقدمة، والتوصيات الغذائية المخصصة.",
    ...
    welcome:
      "👋 مرحباً! أنا **مساعد بيسترو الذكي**. يمكنني مساعدتك في استعراض القائمة، إيجاد أطباق بحسب الذوق، حجز طاولة، أو الإجابة عن أي سؤال!",
    noMatches:
      "لم أجد تطابقاً مباشراً. جرب اسم طبق، أو فئة (بيتزا، مكرونة، برجر)، أو اضغط على أحد الأزرار السريعة!",
    ...
    welcomeFeatures: [
      {
        ...
        desc: "تصفح جميع الأطباق حسب الفئة",
      },
...
const extractMentionedDishes = (text, items) => {
  ...
  return items.filter((item) => {
    ...
    const nameAr = normalizeText(
      globalTranslations.ar?.menuItems?.[item.name]?.name || "",
    );
...
const fuzzyMatch = (query, target, maxDistance = 2) => {
  ...
  return qTokens.some((qt) =>
    tTokens.some((tt) => levenshtein(qt, tt) <= maxDistance),
  );
};
...
const DishCard = ({ item, isArabic, handleAddToCart }) => {
  ...
  const descAr =
    globalTranslations.ar?.menuItems?.[nameEn]?.description || descEn;
  ...
  const catColor =
    categoryColors[categoryName] || "bg-gray-50 text-gray-500 border-gray-100";
  ...
  const imgSrc = !image
    ...
    : parseInt(item.id, 10) >= 1 && parseInt(item.id, 10) <= 27
      ...
      : item.image_url || resolveAssetImage(image);
  ...
  return (
    ...
    >
      ...
      <div className="relative h-36 overflow-hidden bg-gray-50">
        <img
          ...
          onError={(e) => {
            ...
            e.target.src = `https://images.unsplash.com/photo-1565299585323-38d6b0865b47?w=200&q=80`;
          }}
        />
        {/* Category badge */}
        {categoryName && (
          <span
            className={`absolute top-2.5 ${isArabic ? "left-2.5" : "right-2.5"} text-[10px] font-extrabold px-2.5 py-0.5 rounded-full border ${catColor}`}
          >
            {categoryName}
          </span>
        )}
      ...
      {/* Info */}
      <div className="p-3.5">
        <h4 className="font-bold text-[14px] text-[#2C2F34] leading-tight">
          {displayName}
        </h4>
    ...
    return [
      {
        ...
        sender: "bot",
        text:
          botTranslations[localStorage.getItem("lang") || "en"]?.welcome ||
          botTranslations.en.welcome,
  ...
  useEffect(() => {
    ...
    const fetchMenu = async () => {
      try {
        ...
        console.error(
          "Chatbot failed to fetch dynamic menu, using fallback translations",
          err,
        );
  ...
  const getSystemInstruction = () => {
    const menuList = menuItems
      .map((item) => {
        ...
        const nameAr =
          globalTranslations.ar?.menuItems?.[nameEn]?.name || nameEn;
        const descAr =
          globalTranslations.ar?.menuItems?.[nameEn]?.description || descEn;
    ...
    const articlesList = pagesArticles
      .map((art) => {
...
`;
  ...
  const runLocalAgent = (rawText) => {
    ...
    if (lastIntent === "dishes" && lastDishes.length > 0) {
      ...
      if (
        ...
        const sorted = [...lastDishes].sort(
          (a, b) => parseFloat(a.price) - parseFloat(b.price),
        );
        return {
          text: isAr
            ? "إليك الأطباق الأرخص من نتائجك السابقة:"
            : "Here are the cheaper options from your previous results:",
          type: "dishes",
      ...
      if (
        ...
        const sameCat = menuItems.filter(
          (i) =>
            i.category?.name === lastCategory &&
            !lastDishes.some((d) => d.id === i.id),
        );
        if (sameCat.length > 0) {
          return {
            text: isAr
              ? `إليك المزيد من فئة ${lastCategory}:`
              : `Here are more from the ${lastCategory} category:`,
            type: "dishes",
      ...
      if (
        ...
        return {
          text: isAr
            ? "اختر طبقاً لإضافته للسلة:"
            : "Pick one to add to your cart:",
          type: "dishes",
    ...
    if (
      query.match(
        /^(hi|hello|hey|welcome|hola|مرحبا|اهلا|السلام|صباح|مساء|تسلم|شكرا|thanks|thank you)$/,
      )
    ) {
    ...
    if (
      ...
      return {
        ...
        actions: [
          {
            label: isAr ? "📅 انتقل لصفحة الحجز" : "📅 Go to Booking Page",
            path: "/book",
          },
        ],
      };
    ...
    if (
      ...
      return {
        ...
        type: "actions",
        actions: [
          { label: isAr ? "📞 اتصل بنا" : "📞 Contact Page", path: "/contact" },
        ],
      };
    }
    ...
    if (
      ...
      query.includes("خدم") ||
      query.includes("مناسب") ||
      query.includes("حفل") ||
      query.includes("عيد ميلاد") ||
      query.includes("ضياف")
    ) {
      const text = isAr
    ...
    if (
      ...
      return {
        ...
        actions: [
          {
            label: isAr ? "اعرف المزيد عنا 📖" : "Read More About Us 📖",
            path: "/about",
          },
        ],
      };
    ...
    if (
      ...
      return {
        ...
        actions: [
          {
            label: isAr ? "تصفح المقالات 📝" : "Browse Articles 📝",
            path: "/articles",
          },
        ],
      };
    ...
    if (matchedCategories.length > 0) {
      matchedDishes = menuItems.filter((item) =>
        matchedCategories.some(
          (cat) => item.category?.name?.toLowerCase() === cat.toLowerCase(),
        ),
      );
    }

    const queryTokens = query.split(/\s+/).filter((t) => t.length > 2);
    const dishScores = menuItems
      .map((item) => {
        let score = 0;
        const nameEn = normalizeText(item.name);
        ...
        const descAr = normalizeText(
          globalTranslations.ar?.menuItems?.[item.name]?.description || "",
        );
    ...
    if (matchedDishes.length > 0) {
      ...
      return {
        text: isAr
          ? "لقد وجدت هذه الأصناف اللذيذة في قائمتنا والتي تطابق طلبك:"
          : "I found these delicious items on our menu matching your query:",
        type: "dishes",
        dishes: sliceItems,
      };
    }
    ...
    if (
      ...
      query.includes("roulette") ||
      query.includes("حظ") ||
      query.includes("عشوائي") ||
      query.includes("اقترحلي")
    ) {
    ...
    if (
      ...
      query.includes("نباتي") ||
      query.includes("خضار") ||
      query.includes("بدون لحم")
    ) {
      const vegKeywords = [
        "veg",
        "spinach",
    ...
    if (
      ...
      query.includes("رخيص") ||
      query.includes("اقتصادي") ||
      query.includes("سعر منخفض") ||
      query.includes("اقل")
    ) {
      const cheapDishes = menuItems.filter(
        (item) => parseFloat(item.price || 0) < 13,
      );
    ...
    if (
      ...
      return {
        text: isAr
          ? "تصفح قائمتنا أو شاهد بعضاً من أطباقنا الأكثر شعبية وتوصياتنا:"
          : "Browse our menu categories or check out some of our most popular dishes:",
        type: "dishes",
        dishes:
  ...
  const handleAddToCartFromChat = (item) => {
    ...
    const nameAr =
      globalTranslations.ar?.menuItems?.[item.name]?.name || item.name;
          ...
          {isOpen ? (
            ...
            >
              <img
                src={chatbotIcon}
                alt="Chatbot"
                className="w-16 h-16 rounded-full object-cover shadow-lg border-2 border-white hover:border-[#AD343E]/20 transition"
              />
              {unreadCount > 0 && (
          ...
          >
            ...
            <div className="flex-grow flex flex-col relative overflow-hidden bg-[#F9F9F7]/30">
                ...
                {showSettings && (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 text-[10px] font-medium text-gray-500 bg-[#F9F9F7] p-2 rounded-lg border border-gray-100">
                      <Sparkles
                        className={`w-3.5 h-3.5 ${isGeminiActive ? "text-yellow-500 fill-yellow-500" : "text-gray-400"}`}
                      />
                      {isGeminiActive
                        ? tBot("statusGemini")
                        : tBot("statusLocal")}
              ...
              {(() => {
                ...
                const topDishes = Object.entries(stats.topDishes || {})
                  .sort((a, b) => b[1] - a[1])
                  .slice(0, 3);
                    ...
                    {showAnalytics && (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          {[
                            {
                              label: isArabic ? "جلسات" : "Sessions",
                              val: stats.sessionStart || 0,
                              color: "text-blue-400",
                            },
                            {
                              label: isArabic ? "بحث" : "Searches",
                              val: stats.search || 0,
                              color: "text-green-400",
                            },
                            {
                              label: isArabic ? "سلة" : "Cart Adds",
                              val: stats.cartAdd || 0,
                            ...
                            <div
                              key={s.label}
                              className="bg-white/5 rounded-xl p-2.5 text-center border border-white/10"
                            >
                          ...
                          <div>
                            ...
                            <div className="space-y-1.5">
                              {topDishes.map(([name, count]) => (
                                <div
                                  key={name}
                                  className="flex justify-between items-center bg-white/5 rounded-lg px-2.5 py-1.5"
                                >
                ...
                {showClearConfirm && (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="space-y-1.5">
                        <h4 className="font-bold text-sm text-[#2C2F34]">
                          {tBot("clearConfirmTitle")}
                        </h4>
                        <p className="text-[10px] text-gray-400 leading-normal">
                          {tBot("clearConfirmDesc")}
                        </p>
                      </div>
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        ...
                        >
                          ...
                          {(msg.id === "welcome" ? tBot("welcome") : msg.text)
                            .split("\n")
                            .map((line, idx) => (
                          ...
                          {msg.id === "welcome" && (
                            <div className="grid grid-cols-2 gap-2 mt-3.5">
                              ...
                              >
                                ...
                                <span className="font-bold text-[10px] text-[#AD343E] group-hover:text-white">
                                  {isArabic
                                    ? "طبق الحظ / روليت"
                                    : "Food Roulette"}
                                </span>
                                <span className="text-[8.5px] text-gray-500 group-hover:text-white/80 leading-tight">
                                  {isArabic
                                    ? "اختر طبق عشوائي للمغامرة"
                                    : "Pick a random plate"}
                                </span>
                              </button>
                              <button
                              ...
                              >
                                ...
                                <span className="font-bold text-[10px] text-[#AD343E] group-hover:text-white">
                                  {isArabic ? "أطباق نباتية" : "Veg & Healthy"}
                                </span>
                                <span className="text-[8.5px] text-gray-500 group-hover:text-white/80 leading-tight">
                                  {isArabic
                                    ? "أطباق خالية من اللحوم"
                                    : "Meat-free options"}
                                </span>
                              </button>
                              <button
                                onClick={() =>
                                  handleChipClick(
                                    isArabic ? "اكلات حارة" : "Spicy choices",
                                  )
                                }
                                className="p-2.5 bg-[#AD343E]/5 hover:bg-[#AD343E] hover:text-white border border-[#AD343E]/10 rounded-2xl text-start transition duration-200 flex flex-col gap-1 hover:scale-[1.02] active:scale-95 group cursor-pointer"
                              >
                                ...
                                <span className="font-bold text-[10px] text-[#AD343E] group-hover:text-white">
                                  {isArabic ? "أطباق حارة" : "Spicy Specials"}
                                </span>
                                <span className="text-[8.5px] text-gray-500 group-hover:text-white/80 leading-tight">
                                  {isArabic
                                    ? "لمحبي الطعم الحار"
                        ...
                        <span
                          className={`text-[9px] text-gray-400 block ${isBot ? "text-start" : "text-end"}`}
                        >
                          {msg.timestamp}
                        </span>
                      </div>
                ...
                {isLoading && (
                  <div className="flex gap-2.5 max-w-[85%] self-start items-center">
                    <img
                      src={chatbotIcon}
                      alt="Chatbot"
                      className="w-8 h-8 rounded-full object-cover border border-amber-200 flex-shrink-0 animate-bounce"
                    />
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-bold truncate max-w-[200px] mt-0.5">
                          {cartNotification}
                        </p>
                      </div>
                    </div>
              ...
              <div className="border-t border-gray-100/60 bg-white/80 backdrop-blur-sm">
                ...
                <div className="px-3.5 pt-2.5 pb-1.5 overflow-x-auto flex gap-1.5 no-scrollbar">
                  {[
                    {
                      label: isArabic ? "🍕 بيتزا" : "🍕 Pizza",
                      q: isArabic ? "بيتزا" : "pizza",
                    },
                    {
                      label: isArabic ? "🍔 برجر" : "🍔 Burger",
                      q: isArabic ? "برجر" : "burger",
                    },
                    {
                      label: isArabic ? "🍝 مكرونة" : "🍝 Pasta",
                      q: isArabic ? "مكرونة" : "pasta",
                    },
                    {
                      label: isArabic ? "🍰 حلويات" : "🍰 Sweets",
                      q: isArabic ? "حلويات" : "desserts",
                    },
                    {
                      label: isArabic ? "🥤 مشروبات" : "🥤 Drinks",
                      q: isArabic ? "مشروبات" : "drinks",
                    },
                  ].map((chip, i) => (
                    <button
                      key={i}
                ...
                <div className="px-3.5 pb-2.5 overflow-x-auto flex gap-1.5 no-scrollbar">
                  {[
                    {
                      label: tBot("quickChips").menu,
                      q: isArabic ? "وريني قائمة الطعام" : "Show me the menu",
                    },
                    {
                      label: tBot("quickChips").book,
                      q: isArabic
                        ? "ازاي احجز طاولة؟"
                        : "How can I book a table?",
                    },
                    {
                      label: tBot("quickChips").contact,
                      q: isArabic
                        ? "رقم تليفونكم وعنوانكم ايه؟"
                        : "What is your contact info?",
                    },
                    {
                      label: tBot("quickChips").catering,
                      q: isArabic
                        ? "ايه خدمات الضيافة المتاحة؟"
                        : "What catering services do you offer?",
                    },
                    {
                      label: tBot("quickChips").articles,
                      q: isArabic
                        ? "اقترح مقالات اقراها"
                        : "Recommend some blog posts",
                    },
                  ].map((chip, i) => (
                    <button
                      key={i}
              ...
              <div className="relative">
                ...
                >
                  <input
                    ...
                    onKeyDown={(e) => {
                      ...
                      if (e.key === "ArrowDown") {
                        ...
                        setAutocompleteIdx((i) =>
                          Math.min(i + 1, autocompleteList.length - 1),
                        );
                        ...
                        if (autocompleteIdx >= 0) {
                          e.preventDefault();
                          handleSendMessage(
                            autocompleteList[autocompleteIdx].label,
                          );
                        }
                    ...
                    onFocus={() =>
                      autocompleteList.length > 0 && setShowAutocomplete(true)
                    }
                    onBlur={() =>
